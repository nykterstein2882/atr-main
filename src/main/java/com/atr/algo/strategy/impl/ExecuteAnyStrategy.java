...
public class ExecuteAnyStrategy implements AlgoStrategy {
    ...
    public Strategy getStrategyName() {
        return Strategy.EXECUTE_ANY_STRATEGY;
    }
    ...
    private Result.CandleStickResult getCandleStickResult(CandleStick previousDayCandle,
        ...
        if(TradeStatus.NO_TRADE.equals(longResult.getTradeStatus()) && TradeStatus.NO_TRADE.equals(shortResult.getTradeStatus())) {
            ...
            // meaning both trades are made on that day
            // todo: set status accordingly, it doesnt mean the other one is SL hit
            if(longResult.getProfit() > shortResult.getProfit()) {
                longResult.setTradeStatus(TradeStatus.LONG_WIN_SHORT_SL);
                longResult.setProfit(longResult.getProfit() + shortResult.getProfit());
                candleStickResult = longResult;
            } else {
                shortResult.setTradeStatus(TradeStatus.SHORT_WIN_LONG_SL);
                shortResult.setProfit(shortResult.getProfit() + longResult.getProfit());
                candleStickResult = shortResult;
            }
        }
