...
public class ExecuteFirstWithBounds implements AlgoStrategy {
    ...
    public Strategy getStrategyName() {
        return Strategy.EXECUTE_FIRST_WITH_BOUNDS;
    }
    ...
    private Result getResult(List<CandleStick> dailyCandles, Map<String, CandleStick> dailyMap,
        ...
        for ( int i = 0; i < dailyCandles.size() - 1 ; i++ ) {
            ...
            if (!(candleStickResult.getEntryIndex() >= algoConfig.getLowerBound() &&
                    candleStickResult.getEntryIndex() <= algoConfig.getUpperBound())) {
                candleStickResult.setTradeStatus(TradeStatus.NO_TRADE);
                candleStickResult.setRemark("Overriding status to 'No Trade' because entry index not in the range");
            }
            ...
            if (candleStickResult.getTradeStatus() != TradeStatus.NO_TRADE) {
